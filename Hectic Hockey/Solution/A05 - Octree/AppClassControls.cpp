...
void Application::ProcessKeyReleased(sf::Event a_event)
{
	...
	case sf::Keyboard::B:
		m_bShowBS = !m_bShowBS;
		m_pEntityMngr->visibleBS = m_bShowBS;
		for (uint i = 0; i < m_pEntityMngr->GetEntityCount(); i++)
		{
			if (m_pEntityMngr->GetEntity(i)->tag == Puck || m_pEntityMngr->GetEntity(i)->tag == Player) // bounding spheres for pucks & players
				m_pEntityMngr->GetEntity(i)->GetRigidBody()->SetVisibleBS(m_bShowBS);
			else
				m_pEntityMngr->GetEntity(i)->GetRigidBody()->SetVisibleOBB(m_bShowBS); // bounding boxes for goals & walls
		}

		if (m_bShowBS)
			m_pGrid->SetVisible(m_bToggleGrid);
		else
			m_pGrid->SetVisible(m_bShowBS);
		break;
	case sf::Keyboard::G:
		m_bToggleGrid = !m_bToggleGrid;
		SafeDelete(m_pGrid);
		if (!m_bToggleGrid)
			m_pGrid = new MyGrid(1, 0, 1);
		else
	...
	case sf::Keyboard::P:
		for (uint i = 0; i < 100; i++)
		{
			...
			m_pEntityMngr->GetEntity(-1)->GetRigidBody()->SetVisibleBS(m_bShowBS);
			m_uObjects++;
		}
		break;
	case sf::Keyboard::R:
		for (uint i = m_pEntityMngr->GetEntityCount() - 1; i > 0; i--)
		{
			if(m_pEntityMngr->GetEntity(i)->tag == Puck)
				m_pEntityMngr->RemoveEntity(i);
		}	
		m_uObjects = 1; // change to 0 when this actually removes the 5th puck
		m_pEntityMngr->ResetScore();
		for (uint i = 0; i < 4; i++)
		{
			m_pEntityMngr->AddEntity("HockeyModels\\Puck.obj");
			vector3 v3position = vector3(glm::sphericalRand(34.0f));
...
#pragma region Camera Orientation
	...
	// Ignore controller input for camera
